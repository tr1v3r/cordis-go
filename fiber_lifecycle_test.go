...
// TestDisposedWaitsForDeferredTeardown pins the completion signal for a dispose
// that starts while a load body is still running.
func TestDisposedWaitsForDeferredTeardown(t *testing.T) {
	root := cordis.New()
	var runs atomic.Int32
	entered := make(chan struct{})
	releaseBody := make(chan struct{})
	...
	select {
	case <-fiber.Disposed():
		t.Fatal("Disposed closed before the load body finished")
	default:
	}
	...
	case <-fiber.Disposed():
	case <-time.After(2 * time.Second):
		t.Fatal("Disposed was not closed after teardown")
	}
	if fiber.State() != cordis.StateDisposed {
		t.Fatalf("want disposed, got %s", fiber.State())
	}
	if got := len(fiber.Effects()); got != 0 {
		t.Fatalf("effects after teardown = %d, want 0", got)
	}
	if err := <-gate2Done; err != nil {
		t.Fatal(err)
	}
